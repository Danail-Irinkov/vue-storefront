    ...
    <div class="py5 px20">
      <div class="store-info" v-if="brand">
        ...
        <div class="store-contact text shipping-method" :class="{pointer: !isDisabledInputs}" v-if="shippingMethod" @click="showShippingModel(brand._id, isDisabledInputs)">
          ...
          {{ shippingMethod.name }} |&nbsp;
          <span v-if="shippingMethod.cost && shippingMethod.cost > 0">{{ calcShippingCost | mangopay_price }}</span>
...
export default {
  ...
  computed: {
    ...
    calcShippingCost () { // ProCC
      if (this.shippingMethod.offer_free_shipping && this.shippingMethod.free_shipping_amount < this.order_items_total)
        return 0
      else
